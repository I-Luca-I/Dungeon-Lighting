...
class Pawn:
    ...
    def move(self, event:pygame.event.Event, mouse_coords:pygame.Vector2, collision_mask:pygame.Mask) -> None:
        ...
        if (event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and 0 <= x < self.hitbox.get_size()[0] and  0 <= y < self.hitbox.get_size()[1] and self.hitbox.get_at((x, y))):
            self.moving = True
            self.mouse_offset = [int(self.position[0])- mouse_coords[0], int(self.position[1]) - mouse_coords[1]]
        ...
        if len(self.endpoints) > 2:
            pygame.draw.polygon(self.polygon_surface, (255,0,0,255), self.
                                )
        if (self.moving and collision_mask.overlap_area(self.hitbox, (int(mouse_coords[0] - self.texture.get_width()//2), int(mouse_coords[1] - self.texture.get_height()//2))) < 50 and self.polygon_surface.get_at(mouse_coords + self.mouse_offset)) == (255, 0, 0, 255):
