...
class Pawn:
    ...
    def move(self, event:pygame.event.Event, mouse_coords:pygame.Vector2, collision_mask:pygame.Mask) -> None:
        ...
        if (event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and 0 <= x < self.texture.get_size()[
            0] and 0 <= y < self.texture.get_size()[1]):
            self.moving = True
        ...
        for i in range(0, self.max_rays, self.max_rays // 500):
            end = (max(0, min(mouse_coords[0], collision_mask.get_size()[0] - 1)),
                   (max(0, min(mouse_coords[1], collision_mask.get_size()[1] - 1))))
            start = (
                max(0, min(int(end[0] - ((math.sqrt(x ** 2 + y ** 2) + 0) * cos_cache[i])),
                           collision_mask.get_size()[0] - 1)),
                max(0, min(int(end[1] - ((math.sqrt(x ** 2 + y ** 2) + 0) * sin_cache[i])),
                           collision_mask.get_size()[1] - 1))
            )
            self.mouse_startpoints.append(start)

            m = float(start[1] - end[1]) / float(start[0] - end[0]) if (start[0] != end[0]) else \
            collision_mask.get_size()[1]
            q = start[1] - m * start[0]
            dx = abs(start[0] - end[0])
            dy = abs(start[1] - end[1])
        ...
        while (True):
            ...
            self.overlap_vector += self.position - pygame.Vector2(self.texture.get_width() // 2,
            ...
            self.position = pygame.Vector2(max(0, min(self.position[0], collision_mask.get_size()[0] - 1)),
                                           max(0, min(self.position[1], collision_mask.get_size()[1] - 1)))
