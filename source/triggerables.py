...
class Triggerable:
    def __init__(self, masks:dict, coord:pygame.Vector2, states:dict) -> None:
        ...
        existing_triggerables.append(self)
...
class DungeonChanger(Triggerable):
    def trigger(self) -> dict:
        files = os.listdir(f"saves/dungeon_{self.states['destination']['id']}")
        saves = []
        for f in files:
            if f.endswith(".json"):
                saves.append(f[:len("0000-00-00-00-00")])
        saves.sort(reverse=True)
        return {"id":self.states["destination"]["id"], "save":saves[0], "entrance":self.states["destination"]["entrance"]}

    @staticmethod
    def get_changers_mask(surface: pygame.Surface) -> pygame.Mask:
        return Triggerable.get_big_mask(surface=surface, color=(255, 255, 0))

    @staticmethod
    def get_dungeon_changers(changers_mask: pygame.Mask, changers_data:list) -> list:
        dungeon_changers = []
...
class MovingPart(Triggerable):
    @staticmethod
    def get_moving_parts_mask(surface: pygame.Surface) -> pygame.Mask:
        return Triggerable.get_big_mask(surface=surface, color=(0, 0, 0))

    @staticmethod
    def get_moving_parts(moving_parts_mask: pygame.Mask) -> list:
        moving_parts = []
        for instance in Triggerable.get_instances_list(moving_parts_mask, starting_states_dict={"rotate":None, "displace":[None,None]}):
            moving_parts.append(MovingPart(masks=instance.masks, coord=instance.coord, states=instance.states))
        return moving_parts



class Stairs(Triggerable):
    def trigger(self, stairs_list:list, stairs_destinations:list) -> pygame.Vector2:
    ...
    def get_stairs(stairs_mask:pygame.Mask) -> list:
        stairs = []
        for instance in Triggerable.get_instances_list(stairs_mask, starting_states_dict={}):
