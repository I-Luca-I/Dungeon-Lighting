...
class Menu:
    ...
    def run(self) -> dict:
        ...
        while True:
            ...
            if event == "-SAVE-":
                with open(f"saves/dungeon_{values['-DUNGEON-']}/{values['-SAVE-']}_config.json", "r") as file:
                    save_dict = json.load(file)
                for key in save_dict["entrances"]:
                    self.entraces.append(key)
                self.window["-ENTRANCE-"].update(values=self.entraces, value="Scegli un\'entrata", size=(33, 100))  # type: ignore
